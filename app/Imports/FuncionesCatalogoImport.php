...
class FuncionesCatalogoImport
{
    public function import(UploadedFile|string $file): array
    {
        ...
        [$sheetTitle, $rows, $header, $funcionIndex, $diagnostics] = $this->resolveSheetAndColumn($spreadsheet);
        ...
        if ($funcionIndex === null) {
            $message = 'No se encontró una columna válida para funciones. Se esperaba una columna como: nombre_funcion, funcion o nombre.';
            if ($diagnostics !== []) {
        ...
        foreach ($rows as $row) {
            $nombre = $this->normalizeName((string) ($row[$funcionIndex] ?? ''));
            if ($nombre === null) {
                $skipped++;
                continue;
            }

            $existing = FuncionCatalogo::query()->get(['id', 'nombre'])->first(function ($item) use ($nombre) {
            ...
            if ($existing) {
                ...
                FuncionCatalogo::create(['nombre' => $nombre]);
                $inserted++;
            }
        }
    ...
    {
        $accepted = ['nombre_funcion', 'funcion', 'funciones', 'nombre'];
        $diagnostics = [];
        $candidate = [null, [], [], null];
        ...
        foreach ($spreadsheet->getWorksheetIterator() as $sheet) {
            ...
            $funcionIndex = $this->resolveColumnIndex($normalizedHeader, $accepted);
            $diagnostics[] = $sheet->getTitle() . ': fila ' . ($headerIndex + 1) . ' [' . $this->headerPreview($headerRow) . ']';
            $dataRows = array_slice($rows, $headerIndex + 1);

            if ($funcionIndex !== null) {
                return [$sheet->getTitle(), $dataRows, $headerRow, $funcionIndex, $diagnostics];
            }
