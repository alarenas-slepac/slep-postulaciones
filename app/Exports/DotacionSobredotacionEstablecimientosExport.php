...
class DotacionSobredotacionEstablecimientosExport
{
    ...
    private function writeSummary(
        ...
        $generalEstado = $brecha < -0.01 ? 'Sobredotación estructural' : ($brecha > 0.01 ? 'Horas necesarias' : 'Cuadrada');
        $pieEstado = $pieSobredotacion > 0.01 ? 'Sobredotación PIE' : ($pieNecesarias > 0.01 ? 'Horas necesarias' : 'Cuadrada');

        $general = [
            ['Necesidad estructural', $this->hours($aula['horas_necesarias_total'] ?? 0), 'Contrato Aula', $this->hours($aulaFormula['contrato_aula'] ?? 0)],
            ['Resultado', $generalEstado, 'Horas', round(abs($brecha), 2)],
            ['Contrato plan + T. colaborativo', $this->hours($aulaFormula['contrato_plan_pie'] ?? 0), 'Bloque normativo', $this->hours($aulaFormula['bloque_normativo'] ?? 0)],
            ['Bloque declarado', $this->hours($aulaFormula['bloque_declarado'] ?? 0), 'Fórmula', '(plan + colaborativo + normativo + declarado) - contrato Aula'],
        ];
        $aulaRows = [
