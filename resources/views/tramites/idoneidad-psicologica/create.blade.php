...
@section('content')
    <div class="container py-4">
        <section class="ip-create-hero mb-4">
            ...
            <p class="ip-create-help">La búsqueda considera sólo el último padrón vigente de cada establecimiento. Incluirá AAEE cuyo ingreso esté dentro del período y cuyo contrato sea plazo fijo, reemplazo o suplencia.</p>
        </section>

        <section class="ip-create-panel mb-4">
            <div class="ip-create-panel__head"><strong>Período de ingreso del personal</strong><div class="text-muted small">Primero define el rango; luego podrás revisar y marcar la nómina.</div></div>
            <div class="ip-create-panel__body"><form method="GET" action="{{ route('tramites.idoneidad-psicologica.create') }}"><input type="hidden" name="buscar" value="1"><div class="row g-3 align-items-end"><div class="col-md-4"><label class="ip-create-label" for="fecha_inicio">Fecha de inicio</label><input class="form-control @error('fecha_inicio') is-invalid @enderror" type="date" id="fecha_inicio" name="fecha_inicio" value="{{ old('fecha_inicio', $fechas['fecha_inicio']) }}" required>@error('fecha_inicio')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-md-4"><label class="ip-create-label" for="fecha_termino">Fecha de término</label><input class="form-control @error('fecha_termino') is-invalid @enderror" type="date" id="fecha_termino" name="fecha_termino" value="{{ old('fecha_termino', $fechas['fecha_termino']) }}" required>@error('fecha_termino')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-md-4"><button class="ip-create-primary w-100" type="submit"><i class="bi bi-search"></i> Buscar en padrón vigente</button></div></div></form></div>
        </section>
                        ...
                        @else
                            ...
                            <div class="table-responsive ip-candidate-table"><table class="table mb-0"><thead><tr><th class="text-center" style="width:68px">Incluir</th><th>Funcionario</th><th>Establecimiento</th><th>Contrato</th><th>Ingreso</th><th>Término</th></tr></thead><tbody>
                                @foreach($funcionarios as $funcionario)
                                    <tr data-candidato><td class="text-center"><input class="form-check-input candidato-check" type="checkbox" name="funcionarios[]" value="{{ $funcionario->id }}" checked aria-label="Incluir {{ $funcionario->nombre }}"></td><td><div class="ip-person-name">{{ $funcionario->nombre }}</div><div class="ip-person-meta">{{ $funcionario->rut }} · {{ $funcionario->estatuto ?: 'Sin estamento' }} · {{ $funcionario->escalafon ?: 'Sin cargo informado' }}</div></td><td><div class="fw-semibold">{{ $funcionario->establecimiento?->nombre_establecimiento ?: 'Sin establecimiento' }}</div><div class="ip-person-meta">RBD {{ $funcionario->establecimiento?->rbd ?: '—' }}</div></td><td>{{ $funcionario->tipocontrato ?: 'Sin contrato informado' }}</td><td>{{ optional($funcionario->fecha_ingreso)->format('d-m-Y') }}</td><td>{{ optional($funcionario->fecha_termino)->format('d-m-Y') ?: 'Sin fecha' }}</td></tr>
                                @endforeach
                            </tbody></table></div>
