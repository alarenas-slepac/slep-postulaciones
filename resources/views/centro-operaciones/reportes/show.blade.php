...
<div class="co-shell">
    ...
    <div class="co-detail-meta">
        ...
        <div><span>Control de plagas vigente hasta</span><strong>{{ $reporte->fecha_control_plagas?->format('d-m-Y') ?: 'No informado' }}</strong></div>
    </div>
    ...
    <div class="co-grid co-grid--detail">
        <section class="co-card"><div class="co-card-head"><h2>Incidencias del reporte</h2></div><div class="co-list">@forelse($reporte->incidencias as $incidencia)<div class="co-list-item"><span class="co-status-bar co-status-bar--{{ $incidencia->severidad }}"></span><span><strong>{{ $incidencia->tipo_label }}</strong><small>@if($incidencia->modalidad){{ config("centro_operaciones.modalidades_incidencia.{$incidencia->tipo}.{$incidencia->modalidad}", $incidencia->modalidad) }} · @endif{{ $incidencia->descripcion ?: 'Sin detalle' }} · {{ $incidencia->estado === 'resuelta' ? 'Resuelta' : 'Activa' }}</small></span></div>@empty<div class="co-empty">Sin incidencias nuevas en este reporte.</div>@endforelse @if($reporte->incidenciasResueltas->isNotEmpty())<div class="co-list-subtitle">Incidencias anteriores resueltas con este envío</div>@foreach($reporte->incidenciasResueltas as $incidencia)<div class="co-list-item"><span class="co-status-bar co-status-bar--operativo"></span><span><strong>{{ $incidencia->tipo_label }}</strong><small>Resuelta {{ $incidencia->resuelta_en?->format('d-m-Y H:i') }}</small></span></div>@endforeach @endif</div></section>
