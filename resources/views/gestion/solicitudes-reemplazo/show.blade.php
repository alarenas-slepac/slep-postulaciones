...
@section('content')
    ...
    @if ($titularEsDocente && in_array($s->estado, ['aceptada', 'cerrado', 'cerrada'], true) && auth()->user()?->hasAnyRole(['admin','coordinador_gdp','coordinador_gdp_admin','funcionario_slep']))
        <div class="card mb-4 border-primary"><div class="card-header fw-semibold">Resolución docente</div><div class="card-body">
            <div class="small text-muted mb-3">@if ($s->estado === 'aceptada')Debe generarse, revisarse, firmarse y notificarse antes de cerrar esta solicitud.@elseEsta solicitud está cerrada. Puedes cargar la resolución docente firmada para dejarla disponible y notificarla.@endif</div>
            <div class="d-flex flex-wrap gap-2">
                @if ($s->estado === 'aceptada')<form method="POST" action="{{ route('gestion.solicitudes-reemplazo.resolucion-docente.generar', $s) }}">@csrf<button class="btn btn-outline-primary btn-sm">{{ $s->resolucion_docente_docx_path ? 'Regenerar DOCX' : 'Generar DOCX' }}</button></form>@endif
