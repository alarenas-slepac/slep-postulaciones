<nav class="va-nav" aria-label="Navegación del módulo de votaciones">
    ...
    @endcan
    <a href="{{ route('votaciones.admin.incidencias.index', isset($jornada) && $jornada ? ['jornada' => $jornada->slug] : []) }}" @class(['active' => request()->routeIs('votaciones.admin.incidencias.*')])><i class="bi bi-exclamation-triangle"></i> Incidencias</a>
    <a href="{{ route('votaciones.admin.bitacora.index', isset($jornada) && $jornada ? ['jornada' => $jornada->slug] : []) }}" @class(['active' => request()->routeIs('votaciones.admin.bitacora.*')])><i class="bi bi-clock-history"></i> Bitácora</a>
