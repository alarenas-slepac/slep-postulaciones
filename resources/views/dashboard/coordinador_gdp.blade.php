...
@section('content')
    ...
    <div class="dashboard dashboard--coordinador">
        <div class="dashboard-hero mb-4">
            <div class="hero-icon"><i class="bi bi-diagram-2"></i></div>
            <div>
                <h2 class="hero-title h4 m-0">Panel Coordinador GDP</h2>
                <p class="hero-subtitle m-0">Accesos de gestión, derivación y seguimiento operativo</p>
            </div>
        </div>
            ...
            <div class="dashboard-section mb-4">
                ...
                <div class="dashboard-grid">
                    @if ($u->canModule('gestion.solicitudes-reemplazo', $activeRole) && Route::has('gestion.solicitudes-reemplazo.index'))
                        <a href="{{ route('gestion.solicitudes-reemplazo.index') }}" class="text-decoration-none"><div class="tile tile-role"><div class="d-flex align-items-center gap-3"><i class="bi bi-clipboard2-data tile-icon"></i><div><h3 class="h6 m-0 text-dark">Solicitudes de reemplazo</h3><p class="text-muted small m-0">Derivación y seguimiento</p></div></div></div></a>
                    @endif
                    @if ($u->canModule('gestion.estadisticas', $activeRole) && Route::has('gestion.estadisticas.index'))
                        <a href="{{ route('gestion.estadisticas.index') }}" class="text-decoration-none"><div class="tile tile-role"><div class="d-flex align-items-center gap-3"><i class="bi bi-bar-chart-line tile-icon"></i><div><h3 class="h6 m-0 text-dark">Estadísticas</h3><p class="text-muted small m-0">Indicadores y estados de solicitudes</p></div></div></div></a>
                    @endif
        ...
        @if ($u->canModule('reemplazos', $activeRole))
            <div class="dashboard-section mb-4">
                <h3 class="h6 text-uppercase text-muted mb-2">Reemplazos</h3>
                <div class="dashboard-grid">
                    @if (Route::has('reemplazos.index'))
                        <a href="{{ route('reemplazos.index') }}" class="text-decoration-none"><div class="tile tile-role"><div class="d-flex align-items-center gap-3"><i class="bi bi-person-plus tile-icon"></i><div><h3 class="h6 m-0 text-dark">Reemplazos</h3><p class="text-muted small m-0">Consulta del padrón y operación</p></div></div></div></a>
                    @endif
                    @if (Route::has('reemplazos.buscador-postulantes.index'))
                        <a href="{{ route('reemplazos.buscador-postulantes.index') }}" class="text-decoration-none"><div class="tile tile-role"><div class="d-flex align-items-center gap-3"><i class="bi bi-search tile-icon"></i><div><h3 class="h6 m-0 text-dark">Buscador de postulantes</h3><p class="text-muted small m-0">Búsqueda y revisión de ficha</p></div></div></div></a>
                    @endif
                </div>
            </div>
        @endif
