...
if (root) {
    ...
    const labels = { pendiente: 'Pendiente', en_traslado: 'En traslado', en_votacion: 'En votación', finalizada: 'Finalizada', publicada: 'Publicada', en_curso: 'En curso' };
    ...
    const filteredRoutes = () => state.grupos.flatMap((group) => group.rutas.map((route) => ({ ...route, group }))).filter(({ group, ...route }) => {
        return (!selectors.commune.value || route.comuna === selectors.commune.value)
            ...
            && (!selectors.search.value || normalize(`${route.nombre} ${route.rbd}`).includes(normalize(selectors.search.value)));
    });
    ...
    const render = (refreshFilters = false) => {
        ...
        state.grupos.forEach((group, index) => {
            ...
            group.rutas.filter((route) => visibleIds.has(route.id) && Number.isFinite(route.latitud) && Number.isFinite(route.longitud)).forEach((route) => {
